...
BLUE_HSV_LOWER = np.array([95, 110, 80], dtype=np.uint8)
BLUE_HSV_UPPER = np.array([130, 255, 255], dtype=np.uint8)
MIN_BLUE_NAV_AREA_RATIO = 0.0006
...
class VisionRX:
    ...
    def process_frame(self, frame_id, img):
        ...
        blue_navigation = self.detect_blue_navigation(img)
        detection["frame_id"] = frame_id
        detection["time"] = time.time()
        blue_navigation["frame_id"] = frame_id
        blue_navigation["time"] = time.time()
        ...
        self.data["blue_navigation"] = blue_navigation
        self.print_gate_debug(detection, blue_navigation)
        if SHOW_FPV_OVERLAY:
            overlay = self.draw_gate_overlay(img, detection, blue_navigation)
    ...
    def draw_gate_overlay(self, img, detection, blue_navigation=None):
        overlay = img.copy()
        ...
        self._draw_blue_navigation_overlay(overlay, blue_navigation or {})
    ...
    @staticmethod
    def _draw_blue_navigation_overlay(overlay, blue_navigation):
        if not blue_navigation.get("detected"):
            return

        target = blue_navigation.get("target_point")
        anchor = blue_navigation.get("anchor_point")
        if target is None or anchor is None:
            return

        target_pt = (int(round(target[0])), int(round(target[1])))
        anchor_pt = (int(round(anchor[0])), int(round(anchor[1])))
        cv2.line(overlay, anchor_pt, target_pt, (255, 0, 0), 3, cv2.LINE_AA)
        cv2.circle(overlay, target_pt, 7, (255, 80, 80), -1, lineType=cv2.LINE_AA)
        cv2.putText(
            overlay,
            "blue nav",
    ...
    def detect_blue_navigation(self, img):
        height, width = img.shape[:2]
        image_area = max(1, width * height)
        min_blue_pixels = int(image_area * MIN_BLUE_NAV_AREA_RATIO)
        ...
        for contour in contours:
            ...
            is_outer = (
                right_edge >= 0.70
                or left_edge <= 0.30
                or center_x_ratio >= 0.65
                or center_x_ratio <= 0.35
            )
            if not is_outer:
                continue

            # Prefer the right/outside guide for the rightward race direction.
            # The two middle strips pass under the drone and should not steer it.
            right_bias = max(0.0, right_edge - 0.55) * image_area
            edge_distance = abs(center_x - width * 0.5)
            score = area + right_bias + edge_distance * h * 0.02
            outer_candidates.append((score, contour))
        ...
        if not outer_candidates:
            return {
                ...
                "source": "no_outer_strip",
            }

        _, contour = max(outer_candidates, key=lambda item: item[0])
        outer_mask = np.zeros_like(mask)
        cv2.drawContours(outer_mask, [contour], -1, 255, thickness=cv2.FILLED)
        ys, xs = np.where(outer_mask > 0)

        in_flight_path = (ys >= height * 0.06) & (ys <= height * 0.94)
        xs = xs[in_flight_path]
        ys = ys[in_flight_path]
        if len(xs) < min_blue_pixels:
            return {
                "detected": False,
                "confidence": 0.0,
                "pixel_count": int(len(xs)),
                "source": "outer_strip_too_small",
            }

        anchor_band = (xs >= width * 0.25) & (xs <= width * 0.55)
        ahead_band = (xs >= width * 0.55) & (xs <= width * 0.95)
        if np.count_nonzero(anchor_band) < 8:
            anchor_band = np.ones_like(xs, dtype=bool)
        if np.count_nonzero(ahead_band) < 8:
            ahead_band = np.ones_like(xs, dtype=bool)

        anchor_x = float(np.median(xs[anchor_band]))
        anchor_y = float(np.median(ys[anchor_band]))
        target_x = float(np.median(xs[ahead_band]))
        target_y = float(np.median(ys[ahead_band]))
        offset_x = (target_x - (width * 0.5)) / (width * 0.5)
        offset_y = (target_y - (height * 0.5)) / (height * 0.5)
        vertical_trend_y = (target_y - anchor_y) / (height * 0.5)
        area_ratio = float(len(xs) / image_area)
        confidence = max(0.0, min(1.0, area_ratio / 0.006))
        ...
        return {
            ...
            "vertical_trend_y": float(vertical_trend_y),
        }
    ...
    def print_gate_debug(self, detection, blue_navigation=None):
        now = time.time()
        if now - self.last_vision_debug_print_time < VISION_DEBUG_INTERVAL_S:
            return

        self.last_vision_debug_print_time = now
        blue_navigation = blue_navigation or {}
        if blue_navigation.get("detected"):
            trend = float(blue_navigation.get("vertical_trend_y", 0.0))
            if trend < -0.05:
                direction = "up"
            elif trend > 0.05:
                direction = "down"
                ...
                direction = "level"
            blue_text = (
                f"detected  y={blue_navigation.get('offset_y', 0.0):+.2f}  "
                f"trend={trend:+.2f} ({direction})  "
                f"conf={blue_navigation.get('confidence', 0.0):.2f}  "
                f"source={blue_navigation.get('source', 'unknown')}"
            )
        else:
            blue_text = (
                f"none  pixels={blue_navigation.get('pixel_count', 0)}  "
                f"conf={blue_navigation.get('confidence', 0.0):.2f}  "
                f"source={blue_navigation.get('source', 'unknown')}"
            )
        if detection.get("detected"):
