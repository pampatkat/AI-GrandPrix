...
class VisionRX:
    ...
    def __init__(self, data, gate_detector=None, fpv_display=None):
        self.data = data
        self.gate_detector = gate_detector
        self.fpv_display = fpv_display
    ...
    def _vision_loop(self):
        ...
        # Periodically wake up so Ctrl+C shutdown can stop this thread even
        # when the simulator is not sending camera frames.
        ...
        while self.is_running:
            ...
            if len(frames[frame_id]["chunks"]) == total_chunks:
                ...
                if image is not None:
                    self.process_frame(
                        frame_id, image, frames[frame_id]["time"], time.time_ns()
                    )
                else:
    ...
    def process_frame(self, frame_id, img, timestamp_ns=None, received_time_ns=None):
        #
        #
        # Success!
        ...
        if self.gate_detector is not None:
            self.gate_detector.submit(
                frame_id, img, timestamp_ns, received_time_ns=received_time_ns
            )
        if self.fpv_display is not None:
            self.fpv_display.submit(frame_id, img)
