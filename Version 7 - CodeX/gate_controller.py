...
class GateControlConfig:
    ...
    # Extra alignment used only when the drone is close to the gate. Larger
    # size_ratio means the gate is closer in the camera image.
    close_gate_size_ratio: float = 0.35
    close_gate_roll_gain_deg: float = -4.0
    close_gate_roll_limit_boost_deg: float = 1.0
    close_gate_orientation_yaw_gain_deg: float = 0.04
    close_gate_yaw_limit_boost_deg: float = 0.5
    close_gate_thrust_gain: float = 0.35
    # SPEED CONTROL
    # Negative pitch tilts the drone forward. Less negative = slower.
    cruise_forward_pitch_deg: float = -0.50 #control the speed of the drone
    gate_detected_forward_pitch_deg: float = -0.005
    ...
    fast_descent_offset_y_threshold: float = 0.20
    fast_descent_thrust_correction: float = -0.30
    fast_descent_thrust_step: float = -0.05
    fast_descent_release_alpha: float = 0.30
...
class GateController:
    ...
    def _close_gate_blend(self, size_ratio):
        cfg = self._cfg
        if size_ratio <= cfg.close_gate_size_ratio:
            return 0.0
        span = cfg.size_close - cfg.close_gate_size_ratio
        if span <= 0.0:
            return 1.0
        return _clamp((size_ratio - cfg.close_gate_size_ratio) / span, 0.0, 1.0)
    ...
    def _tracking_signals(self, detection):
        """Use the visible gate center for centering; use PnP for angle/size when valid."""
    ...
    def _gate_detected_pitch_deg(self):
        return self._cfg.gate_detected_forward_pitch_deg
    ...
    def update(self, detection, current_velocity_x=0.0):
        ...
        if not self._is_valid_detection(detection):
            ...
            self._fast_descent_bias += cfg.fast_descent_release_alpha * (0.0 - self._fast_descent_bias)
            return GateCommand(
                roll_deg=self._roll_deg,
                pitch_deg=self._smooth_pitch_toward(self._cruise_pitch_deg()),
                yaw_deg=self._yaw_deg,
                thrust_correction=self._fast_descent_bias,
                tracking=False,
                straightening=False,
            )
        ...
        close_blend = self._close_gate_blend(size_ratio)
        ...
        roll_pd = (
            ...
            + cfg.close_gate_roll_gain_deg * offset_x * close_blend
        )
        roll_limit = (
            self._roll_limit_deg(size_ratio)
            + cfg.close_gate_roll_limit_boost_deg * close_blend
        )
        ...
        yaw_limit = cfg.max_orientation_yaw_deg + cfg.close_gate_yaw_limit_boost_deg * close_blend
        yaw_target = _clamp(
            (
                cfg.orientation_yaw_gain_deg * schedule
                + cfg.close_gate_orientation_yaw_gain_deg * close_blend
            ) * orientation_deg,
            -yaw_limit,
            yaw_limit,
        )
        ...
        vertical_gain += cfg.close_gate_thrust_gain * close_blend
        thrust_pd = (
        ...
        if offset_y > cfg.fast_descent_offset_y_threshold:
            self._fast_descent_bias = _clamp(
                self._fast_descent_bias + cfg.fast_descent_thrust_step,
                _clamp(cfg.fast_descent_thrust_correction, -cfg.max_thrust_correction, 0.0),
                0.0,
            )
        else:
            self._fast_descent_bias += cfg.fast_descent_release_alpha * (0.0 - self._fast_descent_bias)
        ...
        return GateCommand(
            roll_deg=self._roll_deg,
            pitch_deg=self._smooth_pitch_toward(self._gate_detected_pitch_deg()),
            yaw_deg=self._yaw_deg,
