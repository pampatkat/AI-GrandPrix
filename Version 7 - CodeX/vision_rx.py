...
GATE_DETECTION_HOLD_S = 1.00
GATE_CANDIDATE_ACCEPT_CONFIDENCE = 0.40
GATE_STABLE_CONFIDENCE_MIN = 0.45
GATE_SMOOTH_ALPHA = 0.45
GATE_SMOOTH_MAX_CENTER_JUMP = 0.60
...
class VisionRX:
    ...
    def stabilize_gate_detection(self, detection):
        ...
        if detection.get("detected"):
            stable = self._smooth_gate_detection(detection)
            stable["raw_detected"] = True
            stable["held_detection"] = False
            stable["confidence"] = max(
                GATE_STABLE_CONFIDENCE_MIN,
                float(stable.get("confidence", 0.0))
            )
            self._last_gate_detection = stable.copy()
            return stable
        ...
        held["confidence"] = max(
            GATE_STABLE_CONFIDENCE_MIN,
            float(held.get("confidence", 0.0)) * 0.9
        )
        return held

    def _smooth_gate_detection(self, detection):
        previous = self._last_gate_detection
        if not previous or not previous.get("detected"):
            return detection

        now = float(detection.get("time", time.time()))
        previous_time = float(previous.get("time", 0.0))
        if now - previous_time < 0.0 or now - previous_time > GATE_DETECTION_HOLD_S:
            return detection

        image_scale = min(
            float(detection.get("image_width", 1.0)),
            float(detection.get("image_height", 1.0)),
        )
        center_error = self.normalized_center_error(
            (float(detection.get("center_x", 0.0)), float(detection.get("center_y", 0.0))),
            (float(previous.get("center_x", 0.0)), float(previous.get("center_y", 0.0))),
            image_scale,
        )
        if center_error > GATE_SMOOTH_MAX_CENTER_JUMP:
            return detection

        smoothed = detection.copy()
        alpha = GATE_SMOOTH_ALPHA
        for key in (
                "center_x",
                "center_y",
                "offset_x",
                ...
                "orientation_deg",
                "distance_estimate_m"):
            current = detection.get(key)
            old = previous.get(key)
            if current is None or old is None:
                continue
            smoothed[key] = (1.0 - alpha) * float(old) + alpha * float(current)

        smoothed["confidence"] = max(
            float(detection.get("confidence", 0.0)),
            float(previous.get("confidence", 0.0)) * 0.95,
        )
        return smoothed
    ...
    def choose_orange_gate_candidate(self, candidates, image_width, image_height):
        ...
        if best_gate is not None and best_gate["confidence"] >= GATE_CANDIDATE_ACCEPT_CONFIDENCE:
