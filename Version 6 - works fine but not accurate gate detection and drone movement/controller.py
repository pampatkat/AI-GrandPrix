...
# --------------------------------------------------------------------------------------
# =============================================================================
# TUNING GUIDE — controller.py
# =============================================================================
# Edit ONLY the constants in "BASELINE FLIGHT SETTINGS" below (@tune: controller.py).
#
# Gate following (left/right, up/down, speed toward gates, detection filters,
# post-pass straighten, etc.) is tuned in gate_controller.py → GateControlConfig.
# Do NOT add GATE_* overrides here — that creates duplicate/dead values.
# =============================================================================

# ======================================================================
# @tune: controller.py ONLY — BASELINE FLIGHT SETTINGS
# ----------------------------------------------------------------------
# These control the drone's own throttle and idle attitude. They do NOT steer
# toward gates. Gate steering comes from GateControlConfig in gate_controller.py.
# ----------------------------------------------------------------------

# Attitude held when no gate is being tracked (e.g. during hover / GO).
# Forward speed during flight comes from gate_controller.py → cruise_forward_pitch_deg.
ROLL_DEG = -0.00  # @tune: controller.py only
YAW_DEG = 0.0  # @tune: controller.py only

# Throttle (0.0 - 1.0). Cruise throttle plus altitude-hold correction; the gate
# follower's vertical correction is added on top, then clamped to min/max below.
BASE_THRUST = 0.30  # @tune: controller.py only — lower cruise keeps gate in view
ALTITUDE_THRUST_GAIN = 0.25  # @tune: controller.py only — pull back toward hold altitude
MIN_FLIGHT_THRUST = 0.10  # @tune: controller.py only
MAX_FLIGHT_THRUST = 0.40  # @tune: controller.py only
...
class Controller:
    def __init__(self, sim_conn, data, system_boot_ms):
        ...
        # Gate follower — all @tune: gate_controller.py values live in GateControlConfig.
        self.gate_config = GateControlConfig()
        self.gate_controller = GateController(self.gate_config)
    ...
    def get_active_gate_detection(self):
        ...
        if not detection.get('detected'):
            return None
        if detection.get('confidence', 0.0) < self.gate_config.confidence_min:
            return None
        if time.time() - detection.get('time', 0.0) > self.gate_config.detection_max_age_s:
            return None
        return detection
