...
# Altitude-hold gain. NOTE: hold_z is latched at launch, so the bigger this is
# the harder it fights a commanded descent toward a low gate (it tries to climb
# back to the launch height). Kept small so the gate's vertical command wins.
ALTITUDE_THRUST_GAIN = 0.03 # how high the drone is above the ground, this is a gain for the altitude control
# Cruise thrust floor. Keeps the drone from sinking while approaching a gate
# that does NOT need a big drop (e.g. the closer second gate flies best with
# this around 0.20). A low far gate, however, needs the drone to fully cut
# thrust to drop in time, which this floor would block -- so it is eased down
# toward MIN_FLIGHT_THRUST_DESCENT automatically whenever the gate controller is
# actively commanding a hard descent (see the descent-aware floor below). That
# lets the closer gate keep its 0.20 floor while the far/low gate still reaches
# ~0.00 to thrust down in time, without hardcoding any gate index.
MIN_FLIGHT_THRUST = 0.20
# Floor used at full commanded descent: the drone may cut thrust this low when
# diving toward a low gate. 0.0 = allow a full cut.
MIN_FLIGHT_THRUST_DESCENT = 0.0
# Commanded descent (magnitude of a negative gate thrust_correction) below which
# the full cruise floor is kept. Small descents (like the closer gate's minor
# corrections) stay at MIN_FLIGHT_THRUST so they don't start sinking.
DESCENT_FLOOR_START = 0.10
# Commanded descent at/above which the floor is fully relaxed to
# MIN_FLIGHT_THRUST_DESCENT. The strong dive toward a far/low gate hits this.
DESCENT_FLOOR_FULL = 0.50
# Must be ABOVE BASE_THRUST or the drone can never climb: the final thrust is
# BASE_THRUST + climb correction, and if this cap equals BASE_THRUST every
# positive (climb) correction is clamped away, leaving only descent authority.
# The headroom here (cap - BASE_THRUST) is the max climb command that survives,
# so keep it at least as large as the gate controller's max_thrust_correction.
MAX_FLIGHT_THRUST = 0.40
...
# Positive: a gate to the RIGHT in the image (offset_x > 0) must command a
# RIGHT bank (+roll) so the drone slides toward the opening. A negative gain
# here banks AWAY from the gate, which made the drone steer off and miss it.
GATE_ROLL_GAIN_DEG = -4.0 # adjust the P and the rate gain adjust the D part

# Lateral-velocity damping: degrees of bank per 1 m/s of measured sideways
# drift. Negative mirrors GATE_ROLL_GAIN_DEG. Make it MORE negative to brake
# sideways drift harder; set to 0.0 to disable. The correction is capped by
# MAX_GATE_LATERAL_VELOCITY_ROLL_DEG, which also sets the bank headroom the
# damping is guaranteed even when MAX_GATE_ROLL_DEG is small.
GATE_LATERAL_VELOCITY_DAMP_DEG = -3.0
MAX_GATE_LATERAL_VELOCITY_ROLL_DEG = 6.0

GATE_ROLL_RATE_GAIN_DEG = 2.0 # D term: matches GATE_ROLL_GAIN_DEG sign so it damps (not anti-damps) the bank
MAX_GATE_ROLL_DEG = 2.5 # maximum bank angle allowed from roll alignment
...
GATE_ORIENTATION_YAW_GAIN_DEG = 0.01 # yaw to match the gate's rotation in the image not left or right.
MAX_GATE_ORIENTATION_YAW_DEG = 0.5 #maximum yaw angle allowed from orientation alignment

GATE_VERTICAL_THRUST_GAIN = 0.25 # vertical thrust gain for the gate controller
...
class Controller:
    def __init__(self, sim_conn, data, system_boot_ms):
        ...
        self.gate_controller = GateController(GateControlConfig(
            ...
            roll_rate_gain_deg=GATE_ROLL_RATE_GAIN_DEG,
            roll_velocity_gain_deg=GATE_LATERAL_VELOCITY_DAMP_DEG,
            max_roll_velocity_correction_deg=MAX_GATE_LATERAL_VELOCITY_ROLL_DEG,
    ...
    def update(self):
        ...
        if self.holding:
            ...
            current_velocity_x = local_velocity.get('x', 0.0)
            # World-NED y is the sideways/lateral axis while the drone flies the
            # course roughly along x (yaw ~0). Feed it so the roll loop can brake
            # accumulated sideways drift before it carries the drone off the gate.
            lateral_velocity = local_velocity.get('y', 0.0)
            ...
            # Altitude-hold thrust. In NED, descending grows current_z, so this
            # term turns positive (it wants to climb back to the launch height).
            altitude_thrust = (current_z - self.hold_z) * ALTITUDE_THRUST_GAIN
            # When the gate controller is commanding a descent toward a low gate
            # (negative correction), don't let altitude-hold add thrust back and
            # fight the dive - it's what makes the drone slow to drop in time.
            if gate_thrust_correction < 0.0:
                altitude_thrust = min(altitude_thrust, 0.0)
            # Symmetrically, when climbing toward a high gate (positive
            # correction), don't let altitude-hold subtract thrust to drag the
            # drone back down to its launch height. hold_z is latched at the
            # start pad, so without this the climb to the (higher) first gate is
            # constantly fought by altitude-hold and the drone stays too low.
            elif gate_thrust_correction > 0.0:
                altitude_thrust = max(altitude_thrust, 0.0)

            # Descent-aware thrust floor. Hold the comfortable cruise floor
            # (MIN_FLIGHT_THRUST) normally, but as the gate controller commands a
            # harder descent (more-negative correction, e.g. diving toward the
            # far/low gate) ease the floor down toward MIN_FLIGHT_THRUST_DESCENT
            # so the drone can fully cut thrust and drop in time. Small descents
            # (the closer gate's minor corrections) keep the full floor.
            effective_min_thrust = MIN_FLIGHT_THRUST
            commanded_descent = -gate_thrust_correction
            if commanded_descent > DESCENT_FLOOR_START:
                span = DESCENT_FLOOR_FULL - DESCENT_FLOOR_START
                t = 1.0 if span <= 0.0 else self._clamp(
                    (commanded_descent - DESCENT_FLOOR_START) / span, 0.0, 1.0
                )
                effective_min_thrust = (
                    MIN_FLIGHT_THRUST
                    + (MIN_FLIGHT_THRUST_DESCENT - MIN_FLIGHT_THRUST) * t
                )
